...
function! kaikyaku#Buster()
  ...
  for f in sort(split(glob(s:data_dir . '/*.xpm'), "\n"))
    ...
    for line in lines[pos1+2:pos2-1]
      ...
      if s[1] == 'NONE'
        call add(colors, printf('syntax match kaikyakuNONE /%s/', join(map(split(s[0], '\zs'), 'printf("[\\x%02x]",char2nr(v:val))'), '')))
        highlight kaikyakuNONE guifg=bg guibg=NONE ctermfg=NONE ctermbg=NONE
      else
        call add(colors, printf('syntax match kaikyaku%s /%s/ contains=kaikyakuNONE', s[1][1:], join(map(split(s[0], '\zs'), 'printf("[\\x%02x]",char2nr(v:val))'), '')))
        exe printf("highlight kaikyaku%s guifg='%s' guibg='%s' ctermfg=%d ctermbg=%d", s[1][1:], s[1], s[1], s:gui2cui(s[1]), s:gui2cui(s[1]))
      endif
    endfor
    call add(images, {
    \ "colors" : colors,
